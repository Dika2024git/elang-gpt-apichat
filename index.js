/* Kode asli dari pengguna (dipertahankan) */
...
const detectKeyword = (input) => {
    let bestMatch = { keyword: null, response: null, score: 0 };

    chatbotData.forEach((data) => {
        data.keywords.forEach((keyword) => {
            let similarity = stringSimilarity.compareTwoStrings(input, keyword);
            if (similarity > bestMatch.score) {
                bestMatch = { keyword, response: data.responses, score: similarity };
            }
        });
    });

    return bestMatch.score > 0.5 ? bestMatch.response : defaultResponses[Math.floor(Math.random() * defaultResponses.length)];
};
...
const getResponse = async (userId, userInput) => {
    ...
    const keywordResponse = detectKeyword(userInput);
    if (keywordResponse) {
        return keywordResponse[Math.floor(Math.random() * keywordResponse.length)];
    }
...
app.get("/apiurl/chat", apiKeyMiddleware, (req, res) => {
    ...
    if (!userInput) {
        return res.json({ reply: "Silakan masukkan pertanyaan dengan /apiurl/chat?text=pertanyaanmu&api_key=your_secret_api_key" });
    }

    const response = getResponse(userId, userInput);
...
/* Perbaikan tambahan */
const defaultResponses = [
    "Aku kurang paham, bisa tanyakan dengan cara lain?",
    "Maaf, aku belum tahu jawabannya.",
    "Aku masih belajar, bisa jelaskan lebih lanjut?",
    "Coba tanyakan sesuatu yang lain, ya!",
    "Hmm, aku belum punya jawaban untuk itu.",
    "Aku nggak yakin, bisa coba pertanyaan lain?"
];
...
const detectKeyword = (input) => {
    const keys = Object.keys(chatbotData);
    const matches = stringSimilarity.findBestMatch(input, keys);
    const bestMatch = matches.bestMatch;

    return bestMatch.rating > 0.3 ? chatbotData[bestMatch.target] : getRandomResponse();
};
...
app.post("/chat", (req, res) => {
    ...
    if (!userData[userId]) {
        userData[userId] = { name: "User", chatHistory: [] };
    }

    const chatHistory = userData[userId].chatHistory;

    if (chatHistory.length >= 3 && chatHistory[0] === chatHistory[1] && chatHistory[1] === chatHistory[2]) {
        return res.json({ reply: "Kamu sudah menanyakan hal ini berkali-kali. Coba sesuatu yang lain!" });
    }

    const response = detectKeyword(message);
